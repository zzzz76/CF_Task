...
class MFRegion(object):
    ...
    def sgd(self, U, W):
        ...
        for uid, iid, r_ui, mean in self.trainset.itertuples(index=False):
            ...
            try:
                ...
                v_i = W[iid]  # 物品向量
                # err 的计算需要修改
                err = np.float32(r_ui - np.dot(v_u, v_i) - (mean + self.bu[uid] + self.bi[iid]))
                ...
                print(U[uid])
                print(W[iid])
                print(np.float32(r_ui - np.dot(U[uid], W[iid]) - (mean + self.bu[uid] + self.bi[iid])))
                print("+++++++++++++++++++")
    ...
    def cost(self, U, W):
        ...
        for uid, iid, r_ui, mean in self.trainset.itertuples(index=False):
            v_u = U[uid]  # 用户向量
            v_i = W[iid]  # 物品向量
            cost += pow(r_ui - np.dot(v_u, v_i) - (mean + self.bu[uid] + self.bi[iid]), 2)
    ...
    def test(self, U, W):
        ...
        for uid, iid, real_rating, mean in self.testset.itertuples(index=False):
            try:
                ...
                pred_rating = mf_g + mean + bias_u + bias_i
